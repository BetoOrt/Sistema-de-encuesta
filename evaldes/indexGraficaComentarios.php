...
///Inicia la gráfica.///Se salta a otra hoja
   $header="<div class='portrait break'>&nbsp;</div>"
 	...
 	$SQL='SELECT comentario FROM '.$cfg['EncuestasAlumnos'].' INNER JOIN ConvertirDeptosaClave ON depto=CDC_CVE INNER JOIN '.$cfg['DDEP'].' ON CDC_DEP=DEP_CVE WHERE folio='.$_POST["grafProf"].' AND TRIM(comentario)<>"" ORDER BY numcues;';
