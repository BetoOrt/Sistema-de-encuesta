...
///Inicia la gráfica.
   ...
   ///Contando total de alumnos del Profesor
	...
	///Preparando los datos para generar una sola ([*Esto puede omitirse*]o dos) gráfica.   
		$contador=0;
		$contadores=array();
		$columnas=array("MATERIA","TOTAL","EVALUARON","FALTANTES","PORCENTAJE","VER GRAFICA");
	   ...
	   {  if($contador==0)
		   {  $numero      =$Registro['Folio'];
			...
			{  $contadores[$contador][$columnas[0]]=$Registro['Materia'];
				...
				if(strlen($Registro['materia2'])>0)
				$contadores[$contador][$columnas[5]]="<a href='javascript:void(0);' id='grafGrupo' folio='$prof' materia='".$Registro['materia2']."'>&nbsp;&nbsp;Grafica por Grupos &nbsp;&nbsp;</a>";
				else
					$contadores[$contador][$columnas[5]]="";
				$contador++;
			}
